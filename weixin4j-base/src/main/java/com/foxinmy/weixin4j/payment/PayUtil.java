...
public class PayUtil {
     ...
     * @param body          订单描述
     * @param orderNo       订单号
     * @param orderFee      订单总额 按实际金额传入即可(元) 构造函数会转换为分
...
//		payPackage.setNotifyUrl(notifyUrl);
        ...
        return createPayJsRequestJson(null, weixinAccount);
    }

    /**
     * 生成V3.x版本JSAPI支付字符串
     *
     * @param payPackage    订单信息
     * @param weixinAccount 商户信息
     * @return 支付json串
     * @throws PayException
     */
...
//		return JSON.toJSONString(jsPayRequest);
     ...
     * @param appId       公众号的ID
     * @param url         当前访问页的URL
     * @param accessToken snsapi_base授权时产生的token
     * @return
     */
     ...
     *
     * @param weixinAccount 支付配置信息
     * @param productId     与订单ID等价
     * @return 支付链接
...
//                timestamp, noncestr);
     ...
     * @param body          商品描述
     * @param outTradeNo    商户内部唯一订单号
     * @param totalFee      商品总额 单位元
     * @param createIp      订单生成的机器 IP
     * @return 支付链接
     * @throws PayException
     * @see <a href="http://pay.weixin.qq.com/wiki/doc/api/native.php">扫码支付</a>
     * @see <a
...
//        return String.format(PayURLConsts.MCH_NATIVE_URL2, prePay.getCodeUrl());
     ...
     * 提交被扫支付
     *
     * @param authCode      扫码支付授权码 ,设备读取用户微信中的条码或者二维码信息
     * @param body          商品描述
     * @param attach        附加数据
     * @param orderNo       商户内部唯一订单号
     * @param orderFee      商品总额 单位元
     * @param ip            订单生成的机器 IP
     * @param weixinAccount 商户信息
     * @return 支付的订单信息
