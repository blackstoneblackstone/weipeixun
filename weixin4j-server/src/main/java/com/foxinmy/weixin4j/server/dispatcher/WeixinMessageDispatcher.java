...
import com.foxinmy.weixin4j.server.response.BlankResponse;
import com.foxinmy.weixin4j.server.response.WeixinResponse;
import com.foxinmy.weixin4j.server.socket.WeixinMessageTransfer;
...
import com.foxinmy.weixin4j.server.xml.MessageTransferHandler;
 ...
 * @see com.foxinmy.weixin4j.server.dispatcher.MessageHandlerExecutor
...
public class WeixinMessageDispatcher {
	...
	public void doDispatch(final ChannelHandlerContext context,
		...
		WeixinMessage message = messageRead(request.getOriginalContent(),
				targetClass);
		logger.debug("define '{}' matched '{}'", messageKey, targetClass);
	...
	protected MessageHandlerExecutor getHandlerExecutor(
		...
		logger.debug("resolve message handlers '{}'", this.messageHandlerList);
		...
		logger.debug("matched message handler '{}'", matchedMessageHandlers);
	...
	public WeixinMessageInterceptor[] getMessageInterceptors()
		...
		logger.debug("resolve message interceptors '{}'",
